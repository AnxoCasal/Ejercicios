...
namespace Ejer2
{
    internal class Aula
    {
        ...
        {
            string[,] result = new string[Alumnos.Length,5];

            for (int i = 0; i < Alumnos.Length; i++)
            {
                result[i, 0] = Alumnos[i];
            }
            ...
            for (int i = 0; i < Notas.GetLength(0); i++)
            {
                for (int j = 0; j < Notas.GetLength(1); j++)
                {
                    result[i, j+1] = Notas[i, j].ToString();
                }
            }
        ...
        public void indicesAlumnos()
        {
            Console.Clear();
            int cont = 1;
            Console.WriteLine("Indeces alumnos");
            foreach (string alumno in Alumnos)
            {
                Console.WriteLine(cont + "-" + alumno);
                cont++;
            }
        }

        public void indicesAsignaturas()
        {
            Console.Clear();
            int cont = 1;
            Console.WriteLine("Indeces Asignaturas");
            for (int i = 0; i < Notas.GetLength(1); i++)
            {
                Console.WriteLine(cont + "-" + (asignaturas)i);
                cont++;
            }
        }
    }
